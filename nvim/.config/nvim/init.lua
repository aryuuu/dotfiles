...
vim.api.nvim_create_user_command("ImportPrevDiary", function()
	...
	table.sort(diary_files, function(a, b)
		local a_date = os.date(
			"%Y-%m-%d",
			os.time({
				year = tonumber(vim.fn.fnamemodify(a, ":t:r")),
				month = tonumber(vim.fn.fnamemodify(a, ":t:r:r")),
				day = tonumber(vim.fn.fnamemodify(a, ":t:r:r:r")),
			})
		)
		local b_date = os.date(
			"%Y-%m-%d",
			os.time({
				year = tonumber(vim.fn.fnamemodify(b, ":t:r")),
				month = tonumber(vim.fn.fnamemodify(b, ":t:r:r")),
				day = tonumber(vim.fn.fnamemodify(b, ":t:r:r:r")),
			})
		)
		return a_date > b_date
			or (a_date == b_date and tonumber(vim.fn.fnamemodify(a, ":t:r:r:r")) > tonumber(
				vim.fn.fnamemodify(b, ":t:r:r:r")
			))
			or (
				a_date == b_date
				and tonumber(vim.fn.fnamemodify(a, ":t:r:r:r")) == tonumber(vim.fn.fnamemodify(b, ":t:r:r:r"))
				and tonumber(vim.fn.fnamemodify(a, ":t:r:r")) > tonumber(vim.fn.fnamemodify(b, ":t:r:r"))
			)
	end)
	...
	{ -- Fuzzy Finder (files, lsp, etc)
		...
		config = function()
			...
			vim.keymap.set(
				"n",
				"<leader>sf",
				"<CMD>Telescope live_grep search= <CR>",
		...
		config = function()
			...
			vim.api.nvim_create_autocmd("LspAttach", {
				...
				callback = function(event)
					...
					map(
						"<leader>lj",
						"<cmd>lua vim.diagnostic.jump({count=1, float=true})<CR>zz",
